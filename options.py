...
def add_replay_options(parser, only_MNIST=False, compare_code="none", **kwargs):
    ...
    if compare_code in ("none"):
        replay.add_argument('--repulsion', action='store_true', dest='repulsion', help="use distribution repulsion with replay")
        replay.add_argument('--lamda-rep', type=float, default=1e-6, dest='repl', help='weight of distribution repulsion loss (def=1)')
        replay.add_argument('--kl-js', type=str, default='js', dest='kl_js', help='use KL or JS divergence for distribution repulsion loss (def=js)')
        replay.add_argument('--use-rep-f', action='store_true', dest='use_rep_factor', help='use repulsion factor (def=False)')
        replay.add_argument('--rep-f', type=float, default=20, dest='rep_factor', help='repulsion factor (def=1.5)')
        ...
        replay.add_argument('--lamda-recon-atr', type=float, default=1e-6, dest='recon_atrl', help='weight of recon attraction loss (def=1e-4)')
